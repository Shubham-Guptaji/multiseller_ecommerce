...
export default function UserInfo({user}: {user: User | null}) {
  ...
  return <div className="">
    <div>
        <Button className="w-full mt-5 mb-4 flex items-center justify-between py-10 truncate" variant="ghost">
            <div className="flex items-center text-left gap-2">
                <Avatar className="w-16 h-16">
                    <AvatarImage src={user?.imageUrl } alt={`${user?.fullName!} ${user?.lastName!}` || "User Avatar"} />
